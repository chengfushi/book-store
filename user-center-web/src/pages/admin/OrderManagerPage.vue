...
import { listOrder, listOrderDetail, cancelOrder } from '@/api/order'; // 假设cancelOrder为删除接口
...
// 页面跳转 - 订单详情
const handleOrderDetail = (order: FrontendOrder) => {
...
const handleDeleteOrder = async (order: FrontendOrder) => {
  ...
  try {
    ...
    const errorMsg = error.response?.data?.message || '删除订单失败，请重试';
